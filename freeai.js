...
const Free = {
  ...
  /* ---------- catalogo de provedores ----------
     ...
     url         base do provedor; por padrão é OpenAI-compatible
     api         adaptador opcional: openai-responses ou anthropic-messages
     site        onde pegar a chave
     gratis      cota/preço informativo na UI — nunca é uma promessa de cota
     ...
     papel       free | top | local; top nunca entra no modo só grátis
  ...
  */
  CAT: [
    ...
    /* ---------- APIs oficiais de fronteira ----------
       Estas duas entradas não são grátis: o modelo é do fabricante e a
       cobrança/accesso pertencem à conta que você cadastrar. O adaptador
       OpenAI usa Responses API; o Anthropic usa Messages API. O catálogo
       vivo (/models) sempre tem prioridade sobre estes nomes de fallback. */
    {
      id: 'openai', nome: 'OpenAI (API oficial)', gratis: 'pago por uso · acesso conforme a sua conta', dia: 0,
      papel: 'top', api: 'openai-responses',
      url: 'https://api.openai.com/v1',
      site: 'https://platform.openai.com/api-keys', chave: 'começa com sk-…', visao: true,
      modelos: ['gpt-6-astra', 'gpt-5.6-sol', 'gpt-5.6-terra', 'gpt-5.6-luna', 'gpt-5.5', 'o3'],
      tirar: /embedding|moderation|tts|transcri|realtime|audio|image|search-preview/i,
    },
    {
      id: 'anthropic', nome: 'Anthropic (Claude API)', gratis: 'pago por uso · acesso conforme a sua conta', dia: 0,
      papel: 'top', api: 'anthropic-messages',
      url: 'https://api.anthropic.com/v1',
      site: 'https://console.anthropic.com/settings/keys', chave: 'começa com sk-ant-…', visao: true,
      modelos: ['claude-fable-5-1', 'claude-opus-5', 'claude-fable-5', 'claude-sonnet-5', 'claude-haiku-4-5'],
      tirar: /embed|moderation|tts|whisper|audio/i,
    },
    {
  ...
  /* Cabeçalhos variam entre as APIs oficiais. Nunca mandamos uma chave de
     um provedor para outro: `pr` já foi produzido para a conta escolhida. */
  _headers(id, pr) {
    const p = this.prov(id) || {};
    const cab = { 'Content-Type': 'application/json' };
    if (p.api === 'anthropic-messages') {
      if (pr.token && pr.token !== 'local') cab['x-api-key'] = pr.token;
      cab['anthropic-version'] = '2023-06-01';
      /* O navegador é opcional no Anthropic. Quando a API aceitar chamadas
         diretas, este cabeçalho habilita o modo explicitamente perigoso; em
         produção, prefira um proxy seu para não expor a chave no cliente. */
      cab['anthropic-dangerous-direct-browser-access'] = 'true';
    } else if (pr.token && pr.token !== 'local') {
      cab.Authorization = 'Bearer ' + pr.token;
    }
    if (id === 'openrouter') {
      const origem = (typeof location !== 'undefined' && location.origin) ? location.origin : 'https://arkher.local';
      cab['HTTP-Referer'] = origem;
      cab['X-Title'] = 'ARKHER';
    }
    return cab;
  },
  ...
  _maxTokens() {
    const n = Number(LS.get('arkher_max_tokens', 2048));
    return Number.isFinite(n) && n >= 128 ? Math.min(32000, Math.floor(n)) : 2048;
  },

  _anthropicBody(modelo, messages, stream) {
  ...
  _responsesBody(modelo, messages, stream) {
    return {
      model: modelo,
      input: messages || [],
      max_output_tokens: this._maxTokens(),
      stream: !!stream,
    };
  },
  ...
  /* ---------- chamada: OpenAI-compatible, Responses ou Anthropic ---------- */
  async ask(id, modelo, messages, onDelta) {
    ...
    const api = p.api || 'openai-chat';
    const sufixo = api === 'anthropic-messages' ? '/messages'
      : api === 'openai-responses' ? '/responses' : '/chat/completions';
    const corpo = api === 'anthropic-messages'
      ? this._anthropicBody(modelo, messages, !!onDelta)
      : api === 'openai-responses'
        ? this._responsesBody(modelo, messages, !!onDelta)
        : { model: modelo, messages, stream: !!onDelta, max_tokens: this._maxTokens() };
    const url = this._url(id, sufixo, pr);
    let r;
    try {
      r = await fetch(url, { method: 'POST', headers: this._headers(id, pr), body: JSON.stringify(corpo) });
    } catch (e) {
      const httpsLocal = typeof location !== 'undefined' && /^https:/.test(location.protocol) && p.local;
      throw new Error(p.nome + ': nao consegui falar com ' + this._url(id, '', pr)
        + (httpsLocal ? ' (pagina HTTPS x servidor local: use o Chrome/Edge, ou um nó seu)' : '')
        + ' — ' + (e.message || e));
    }
    if (!r.ok) {
      let d = '';
    ...
    if (onDelta && r.body && r.body.getReader) {
      texto = await this._sse(r.body, onDelta, api);
  ...
  /* leitor de SSE para as três famílias de API. */
  async _sse(corpo, onDelta, api) {
    ...
    const consumir = linha => {
      ...
      let j; try { j = JSON.parse(dado); } catch (e) { return; }
      let t = '';
      if (api === 'anthropic-messages' && j.type === 'content_block_delta') t = j.delta?.text || '';
      else if (api === 'openai-responses' && j.type === 'response.output_text.delta') t = j.delta || '';
      else if (api !== 'anthropic-messages' && api !== 'openai-responses') t = this._texto(j);
    ...
    for (;;) {
      ...
      for (const l of linhas) consumir(l);
    }
    if (buf) consumir(buf);
    return cheio;
  },
  ...
  _texto(j) {
    ...
    if (j.message && j.message.content !== undefined) return this._texto({ content: j.message.content });
    if (typeof j.content === 'string') return j.content;
    if (Array.isArray(j.content)) return j.content.map(p => typeof p === 'string' ? p : (p && (p.text || p.content)) || '').join('');
    if (Array.isArray(j.output)) {
      return j.output.map(item => Array.isArray(item.content)
        ? item.content.map(p => p && (p.text || p.output_text) || '').join('') : '').join('');
    }
    return '';
  },
