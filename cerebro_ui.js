...
(function () {
  ...
  function mountCerebro() {
    ...
    c5.append(r5);
    /* A estação só registra jobs iniciados pelo usuário. Ela não dispara
       treino remoto por conta própria; o worker autorizado publica eventos
       e um avaliador separado pode chamar evaluate() depois. */
    function iniciarJobTreino(nome, base, meta) {
      if (typeof ArkherStation === 'undefined') return null;
      const workerId = 'gpu-worker-' + String(base || 'local').replace(/[^a-z0-9]+/ig, '-').slice(0, 50);
      ArkherStation.registerWorker({ id: workerId, name: 'worker GPU autorizado', kind: 'remote', authorized: true, capabilities: ['train', 'checkpoint', 'evaluate'] });
      const run = ArkherStation.createRun({ name: nome, kind: 'lora', base: base || 'modelo-base', notes: 'iniciado manualmente no Cérebro' });
      const job = ArkherStation.enqueue({ kind: 'train', runId: run.id, priority: 10, requestedBy: 'user', payload: { name: nome, passos: meta && meta.passos, base: base || 'modelo-base' } });
      ArkherStation.claim(job.id, workerId);
      ArkherStation.event({ type: 'train_start', runId: run.id, jobId: job.id, workerId, state: 'running' });
      return { runId: run.id, jobId: job.id, workerId };
    }
    function eventoJobTreino(j, extra) {
      if (j && typeof ArkherStation !== 'undefined') {
        const e = Object.assign({ type: 'train_telemetry', runId: j.runId, jobId: j.jobId, workerId: j.workerId }, extra || {});
        ArkherStation.event(e);
        if (e.progress != null || (e.metrics && Object.keys(e.metrics).length)) {
          ArkherStation.checkpoint(j.runId, { progress: e.progress, metrics: e.metrics || {}, remoteJobId: e.remoteJobId, message: e.message || '' });
        }
      }
    }
    function finalizarJobTreino(j, ok, extra) {
      if (!j || typeof ArkherStation === 'undefined') return;
      eventoJobTreino(j, Object.assign({ type: ok ? 'train_finished' : 'train_failed', state: ok ? 'trained' : 'failed' }, extra || {}));
      ArkherStation.complete(j.jobId, extra || {}, ok ? 'completed' : 'failed');
      ArkherStation.updateRun(j.runId, Object.assign({ status: ok ? 'trained' : 'failed' }, extra || {}));
    }
    ...
    r5b.append(botao('Treinar no nó e usar no chat', 'pri', async () => {
      ...
      const passos = parseInt(ipassos.value) || 60;
      const jobEstacao = iniciarJobTreino(nome, baseHf(selb.value), { passos });
      logar($('#cb-log'), 'mandando ' + jsonl.split('\n').length + ' exemplos pro nó...'
        + (jobEstacao ? ' · job ' + jobEstacao.jobId + ' registrado na estação' : ''));
      try {
        ...
        const r = await fetch(base + '/treinar', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ jsonl, base: baseHf(selb.value), nome, passos }),
        });
        const j = await r.json();
        if (!j.ok) throw new Error(j.err || 'o nó recusou');
        if (jobEstacao) eventoJobTreino(jobEstacao, { remoteJobId: j.id, gpu: j.gpu || 'GPU', state: 'running' });
        ...
        for (let i = 0; i < 240; i++) {
          ...
          if (jobEstacao) eventoJobTreino(jobEstacao, { remoteJobId: j.id, state: st2.estado || 'running', progress: st2.progresso, metrics: st2.metricas || st2.metrics || {}, message: (st2.linhas || []).slice(-1)[0] || '' });
          if (st2.estado === 'rodando') continue;
          if (st2.estado === 'pronto') {
            finalizarJobTreino(jobEstacao, true, { remoteJobId: j.id, artifacts: ['gpu:local:' + nome], metrics: st2.metricas || st2.metrics || {}, notes: 'treino concluído; aguardando avaliação independente' });
            logar($('#cb-log'), 'TREINOU. candidato gpu:local:' + nome + ' registrado; avalie e promova manualmente no painel da estação antes de usar em produção');
          } else {
            finalizarJobTreino(jobEstacao, false, { remoteJobId: j.id, error: st2.erro || 'treino não concluiu', notes: st2.como_resolver || '' });
            logar($('#cb-log'), 'falhou: ' + (st2.erro || '?') + ' ' + (st2.como_resolver || ''));
          }
          treinados(); mostrarStats(); break;
        }
      } catch (e) { finalizarJobTreino(jobEstacao, false, { error: e.message || String(e) }); logar($('#cb-log'), 'treino: ' + e.message); }
    }));
    c5.append(r5b);
