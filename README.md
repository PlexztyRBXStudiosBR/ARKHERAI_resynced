...
## Operação pesada e preflight

O agente agora inicia em **modo seguro**: `ARKHER_AGENT_TOKEN` é obrigatório para
qualquer operação, CORS só é emitido para `ARKHER_CORS_ORIGINS` explícito e
`/ready` retorna 503 até a autenticação estar configurada. `/health` permanece
público somente para supervisores. Rotas de arquivo ficam em `ARKHER_STATE/work`
(+ raízes declaradas) e têm limites de request, output, escrita e download.
Comandos remotos continuam sendo execução deliberada em uma conta/VM dedicada;
a autenticação não substitui um sandbox de sistema operacional.

Antes de uma sessão longa, rode:

```bash
python3 arkher_doctor.py --json --agent http://127.0.0.1:8765 \
  --token "$ARKHER_AGENT_TOKEN"
python3 teste_hardening.py
python3 teste_carga_agent.py --requests 120 --concurrency 32
```

O doctor verifica checkout, sintaxe Python/JavaScript, escrita e espaço do
estado, JSON persistido, token e readiness. Para Linux, os templates de
supervisão com `Restart=on-failure`, cgroups e workspace estão em `deploy/`.
Para executar jobs autorizados, use um worker opt-in com uma raiz isolada:

```bash
python3 arkher_station_worker.py --agent http://127.0.0.1:8765 \
  --token "$ARKHER_AGENT_TOKEN" --workspace /projetos/autorizados \
  --allow-game-dev --kinds game-dev
```

Leases expirados são reencaminhados até o limite; jobs podem pedir retry
explícito (`retryOnFailure`), podem ser cancelados por `/station/job/cancel` e
são confirmados somente com resultado observado e artefato medido por SHA-256.
Após reinício, processo em memória é recuperado como `aborted`, nunca como
sucesso. Ausência de Blender/Godot/Unity/Unreal, GUI/login do Roblox, GPU e
qualidade visual ainda exigem validação real no hardware autorizado.
...
3. **Secrets do Actions** (Settings -> Secrets -> Actions):
   - `ARKHER_AGENT_TOKEN` — segredo aleatório nosso, gerado localmente com
     `python3 -c "import secrets; print(secrets.token_urlsafe(32))"`;
   - `TAILSCALE_AUTH_KEY` — auth key do Tailscale, com validade/tag restrita;
   - `PAT_TOKEN` — fine-grained PAT limitado ao repositório com Actions Read/write
     (ou clássico com escopo `workflow` se necessário; ele religa a VM);
   - opcional `KAGGLE_USERNAME` + `KAGGLE_API_TOKEN` (ou `KAGGLE_KEY` legado).
