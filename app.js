/* ============================================================
   ...
   ============================================================ */
'use strict';
/* módulos do primeiro incremento vertical: no navegador são carregados pelo
   index.html; no Node entram aqui para que o núcleo também seja testável. */
if (typeof Guard === 'undefined' && typeof require !== 'undefined') {
  try { globalThis.Guard = require('./arkher_guard.js').Guard; } catch (e) {}
}
if (typeof ArkherTask === 'undefined' && typeof require !== 'undefined') {
  try { globalThis.ArkherTask = require('./arkher_task.js').ArkherTask; } catch (e) {}
}
...
const Arkher = {
  ...
  async ask(messages, opts = {}) {
    ...
    const streamRequested = opts.onDelta || null;
    /* A resposta só chega à interface depois do Guard. Isso evita mostrar
       durante o streaming algo que precisaria ser redigido/bloqueado depois. */
    const onDelta = (typeof Guard !== 'undefined' && opts.guard !== false) ? null : streamRequested;
    const maxTries = opts.maxTries || 25;
    const promoted = (typeof ArkherStation !== 'undefined' && ArkherStation.snapshot)
      ? ArkherStation.snapshot().activeModel : null;
    const forced = opts.model || (promoted && promoted.model) || undefined;
    const modo = opts.modo || this.modo();      // auto | gratis | completo
    const soGratis = modo === 'gratis' || opts.gratis === true;

    /* ---------- primeiro portão: classificar, redigir segredos e rotear ---------- */
    const originalMessages = Array.isArray(messages) ? messages : [];
    const ultimoUsuario = (originalMessages.filter(m => m && m.role === 'user').pop() || {}).content || '';
    const task = (typeof ArkherTask !== 'undefined')
      ? ArkherTask.detect(ultimoUsuario)
      : { id: stage === 'code' ? 'engineering' : 'general', tags: [], verificar: stage === 'code', pesquisar: false };
    const tarefaVisual = !!(task.id === 'creative-tech' || (task.tags || []).some(x => ['blender', 'visual', 'cascadeur'].includes(x)));
    if (typeof ArkherStation !== 'undefined') {
      try { ArkherStation.event({ type: 'request_start', task: task.id, tags: task.tags, stage }); } catch (e) {}
    }
    if (typeof Guard !== 'undefined' && opts.guard !== false) {
      const entrada = Guard.classificar(ultimoUsuario, opts.contexto || '');
      if (entrada.nivel === 'bloquear') {
        const text = Guard.alternativa(entrada.riscos);
        try { ArkherStation && ArkherStation.event(Guard.event({ action: 'block', riscos: entrada.riscos }, { type: 'guard', task: task.id })); } catch (e) {}
        return { text, model: 'guard', src: 'guard', prov: 'guard', viaLivre: true,
                 gastouSaldo: false, tried: 0, guard: { action: 'block', riscos: entrada.riscos } };
      }
    }
    /* Copia e redige tokens antes de qualquer provedor. Não mutamos o
       histórico da UI, e o segredo não entra no prompt de terceiros. */
    messages = originalMessages.map(m => {
      if (!m || typeof m !== 'object') return m;
      if (typeof m.content !== 'string' || typeof Guard === 'undefined' || opts.guard === false) return Object.assign({}, m);
      return Object.assign({}, m, { content: Guard.sanear(m.content).text });
    });
    if (typeof ArkherTask !== 'undefined' && task.id !== 'general' && opts.taskContext !== false) {
      const contexto = ArkherTask.contexto(task, ultimoUsuario);
      if (contexto && !messages.some(m => m && m.role === 'system' && String(m.content || '').includes('MODO DE TRABALHO ARKHER'))) {
        messages = [{ role: 'system', content: contexto }].concat(messages);
      }
    }
    ...
    try {
      if (opts.cache !== false && typeof Resp !== 'undefined' && Resp.posso(messages)) {
        ...
        if (it) {
          ...
          let cv = null;
          if (typeof Guard !== 'undefined' && opts.guard !== false) {
            cv = Guard.verificar(ultimoUsuario, ct, { model: it.m || 'cache', task: task.id });
            ct = cv.text;
            try { ArkherStation && ArkherStation.event(Guard.event(cv, { type: 'guard_cache', task: task.id })); } catch (e) {}
          }
          try { this.metricaPedido(false); } catch (e) {}     // conta como grátis
          return { text: ct, model: (it.m || 'resposta do site'), src: cv && cv.action === 'block' ? 'guard' : 'cache',
                   prov: 'site', viaLivre: true, gastouSaldo: false, tried: 0, doCache: true, task, guard: cv };
        }
      }
    } catch (e) { chaveCache = ''; }
    ...
    if (forced) {
      ...
      if ((opts.preferirGPU || tarefaVisual) && (r.gpu || []).length) {
    ...
    for (const item of fila) {
      ...
      try {
        ...
        /* Portão final: uma resposta de um provedor nunca é entregue como
           fato só porque veio com HTTP 200. Segredos são redigidos, conteúdo
           de alto risco é substituído por alternativa segura e código recebe
           a marca "precisaTeste" para a UI/loop verificarem. */
        let verdict = null;
        if (typeof Guard !== 'undefined' && opts.guard !== false) {
          verdict = Guard.verificar(ultimoUsuario, t, { model: item.id, task: task.id });
          if (verdict.action === 'block') {
            try { ArkherStation && ArkherStation.event(Guard.event(verdict, { type: 'guard_block_output', task: task.id, model: item.id })); } catch (e) {}
            return { text: verdict.text, model: 'guard', src: 'guard', prov: 'guard', viaLivre: true,
                     gastouSaldo: false, tried, guard: verdict };
          }
          t = verdict.text;
          try { ArkherStation && ArkherStation.event(Guard.event(verdict, { type: 'guard_verdict', task: task.id, model: item.id })); } catch (e) {}
        }
        ...
        const resposta = { text: t, model: item.id, src: item.src, prov: item.prov, viaLivre: !!item.livre,
          gastouSaldo: !(item.src === 'free' || item.livre || item.src === 'gpu'), tried,
          task, guard: verdict || null };
        try {
          if (typeof ArkherStation !== 'undefined') {
            ArkherStation.event({ type: 'response', task: task.id, tags: task.tags, model: item.id,
              src: item.src, tried, gastouSaldo: resposta.gastouSaldo,
              verified: !!(verdict && verdict.precisaTeste === false), warnings: verdict ? verdict.avisos : [] });
            ArkherStation.experience({ task: task.id, model: item.id, outcome: 'response',
              verified: !(verdict && verdict.precisaTeste), warnings: verdict ? verdict.avisos : [] });
          }
        } catch (e) {}
        return resposta;
      } catch (e) {
        const msg = _err(e);
        lastErr = msg;
        try { if (typeof ArkherStation !== 'undefined') ArkherStation.event({ type: 'provider_error', task: task.id, model: item.id, src: item.src, error: msg.slice(0, 300) }); } catch (x) {}
        if (item.src === 'puter') {
