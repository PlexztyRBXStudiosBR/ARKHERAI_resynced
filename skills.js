...
async function runTool(call, onLog) {
  try {
    ...
    try {
      if (typeof ArkherStation !== 'undefined') ArkherStation.event({ type: 'tool_result', tool: call && call.tool, ok: !!(r && r.ok),
        outputSize: r && r.out ? String(r.out).length : 0, error: r && !r.ok ? String(r.err || '').slice(0, 300) : '' });
    } catch (e) {}
    return r;
  } catch (e) {
    try { if (typeof ArkherStation !== 'undefined') ArkherStation.event({ type: 'tool_error', tool: call && call.tool, error: String(e.message || e).slice(0, 300) }); } catch (x) {}
    throw e;
  }
}
