...
(async function () {
  ...
  /* roda mesmo: sobe um fixture HTTP local próprio. Assim o teste não
     depende de outro servidor já estar rodando na porta 8000. */
  let rodou = false, saida = '', servidor = null;
  try {
    const htmlFixture = '<html><body><h1>Doc de teste</h1><p>'
      + ('conhecimento util sobre o cofre neural e testes de game dev. '.repeat(80))
      + '</p></body></html>';
    ...
    const py2 = Worker.gerar({ seeds: ['http://127.0.0.1:' + port + '/'], paginas: 1, depth: 0, pausa: 0.05, saida: '/tmp/pack_worker_teste.md' }).script;
    fs.writeFileSync('/tmp/arkher_worker_run.py', py2);
    const exec = await execFileAsync('python3', ['/tmp/arkher_worker_run.py'], { timeout: 40000, encoding: 'utf8' });
    saida = exec.stdout || '';
    rodou = fs.existsSync('/tmp/pack_worker_teste.md');
  } catch (e) { rodou = false; }
  finally { if (servidor) await new Promise(resolve => servidor.close(resolve)); }
  if (rodou) {
