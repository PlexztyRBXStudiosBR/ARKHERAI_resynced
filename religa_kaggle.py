...
Token (uma das formas):
  1) token atual: KAGGLE_API_TOKEN (KGAT_...) + KAGGLE_USERNAME
  2) formato legado: KAGGLE_USERNAME + KAGGLE_KEY
  3) arquivo ~/.kaggle/kaggle.json {"username":"...","key":"..."}
...
TOKEN_URL = 'https://www.kaggle.com/settings/api'
...
def _tem_token():
    if os.environ.get('KAGGLE_API_TOKEN') and os.environ.get('KAGGLE_USERNAME'):
        return True
...
def main():
    ...
    if not cmd:
        print(json.dumps({'ok': False, 'erro': 'o CLI do kaggle nao esta instalado',
                          'como_resolver': 'pip install kaggle  (e coloque KAGGLE_API_TOKEN + KAGGLE_USERNAME '
                                           'ou KAGGLE_USERNAME/KAGGLE_KEY, ou salve ~/.kaggle/kaggle.json — pegue em %s)' % TOKEN_URL},
                         ensure_ascii=False, indent=1))
        return 2
    if not _tem_token():
        print(json.dumps({'ok': False, 'erro': 'sem credencial do Kaggle',
                          'como_resolver': 'KAGGLE_API_TOKEN + KAGGLE_USERNAME (ou KAGGLE_USERNAME/KAGGLE_KEY ou ~/.kaggle/kaggle.json). '
