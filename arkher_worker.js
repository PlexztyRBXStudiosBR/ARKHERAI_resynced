...
`;
  ...
  async passivo(quantas, onLog) {
    ...
    const station = (typeof ArkherStation !== 'undefined') ? ArkherStation : null;
    const workerId = 'browser-passive-ingest';
    let job = null;
    if (station) {
      station.registerWorker({ id: workerId, name: 'ingestão passiva deste navegador', kind: 'local', authorized: true, capabilities: ['download', 'index'] });
      job = station.enqueue({ kind: 'ingest', priority: 1, requestedBy: 'user', payload: { paginas: quantas || 4 } });
      station.claim(job.id, workerId);
    }
    try {
      ...
      if (station && job) {
        station.event({ type: 'ingest_complete', workerId, jobId: job.id, metrics: { pages: out.baixadas, blocks: out.blocos, errors: (out.erros || []).length } });
        station.complete(job.id, out, out.ok ? 'completed' : 'failed');
      }
      return out;
    } catch (e) {
      if (station && job) {
        station.event({ type: 'ingest_error', workerId, jobId: job.id, error: String(e.message || e).slice(0, 300) });
        station.complete(job.id, { error: String(e.message || e).slice(0, 300) }, 'failed');
      }
      throw e;
    }
  },
