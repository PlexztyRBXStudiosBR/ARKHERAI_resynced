...
/* Autenticação opcional do agente: um único wrapper cobre as chamadas HTTP
   espalhadas pelos adapters antigos. Sem token, mantém compatibilidade; com
   token, só as URLs configuradas do agente recebem o Bearer. */
(function instalarAuthAgente() {
  if (!window.fetch || window.__arkherAgentAuth) return;
  const originalFetch = window.fetch.bind(window);
  window.fetch = function (input, init) {
    try {
      const token = (window.LS && LS.get('arkher_agent_token', '')) || '';
      const rawUrl = typeof input === 'string' ? input : (input && input.url) || '';
      const alvo = new URL(rawUrl, location.href);
      const bases = ['arkher_agent', 'arkher_kaggle', 'arkher_droid']
        .map(k => (window.LS && LS.get(k, '')) || '').filter(Boolean);
      const protegido = bases.some(base => {
        try { return new URL(base, location.href).origin === alvo.origin; } catch (e) { return false; }
      });
      if (token && protegido) {
...
async function send() {
  ...
  try {
    ...
    const perfil = (window.ArkherTask && ArkherTask.detect) ? ArkherTask.detect(text) : null;
    const etapa = perfil ? perfil.stage : (/codigo|code|script|programa|lua|python|erro/i.test(text) ? 'code' : 'chat');
    if (perfil && perfil.id !== 'general') {
      out.head.innerHTML = `ARKHER <span class="tag">${perfil.id} · preparando verificação</span>`;
    }
    const r = await Arkher.ask(msgs, {
      ...
      onTry: (it, n) => {
        ...
        out.head.innerHTML = `ARKHER <span class="tag">${rotuloFonte(it)} · ${perfil ? perfil.id + ' · ' : ''}tentativa ${n}</span>`;
      },
      onDelta: t => { out.body.textContent += t; logEl.scrollTop = logEl.scrollHeight; },
    });
    const precisaRevisao = !!(r.guard && (r.guard.precisaTeste || (r.guard.avisos || []).includes('revisao-autorizada')));
    out.body.textContent = r.text;
    out.head.innerHTML = `ARKHER <span class="tag">${rotuloFonte(r)} · ${r.model.split('/').pop()}${precisaRevisao ? ' · verificar' : ''}</span>`;
    if (window.pintarUso) pintarUso();
    if (window.Memoria) {           // o que rolou nesta sessao fica pro proximo chat/aba/VM
      try {
        const textoSeguro = (window.Guard && Guard.sanear) ? Guard.sanear(text).text : text;
        Memoria.add({ tipo: 'nota', texto: 'pergunta: ' + textoSeguro.slice(0, 200),
    ...
    for (let volta = 0; volta < 3; volta++) {
      ...
      let res;
      try {
        const toolGate = (window.Guard && Guard.autorizarFerramenta)
          ? Guard.autorizarFerramenta(call, text) : { ok: true };
        if (!toolGate.ok) {
          res = { ok: false, err: 'Guard bloqueou a ferramenta: ' + toolGate.reason };
          try { if (window.ArkherStation) ArkherStation.event({ type: 'guard_tool_block', tool: call.tool, reason: toolGate.reason }); } catch (x) {}
        } else res = await runTool(call, t => { box.body.textContent = t; });
      }
...
if ($('#f-agent-token')) $('#f-agent-token').value = LS.get('arkher_agent_token', '');
$('#b-savehf').onclick = () => {
...
$('#b-saveagent').onclick = () => {
  ...
  if ($('#f-agent-token')) LS.set('arkher_agent_token', $('#f-agent-token').value.trim());
  $('#f-agent').value = LS.get('arkher_agent', '');
...
async function pingAgent() {
  ...
  try {
    ...
    if (j.ready === false && j.auth_required) {
      d.className = 'dot bad'; s.textContent = 'token do agente ausente';
      return false;
    }
    d.className = 'dot on';
