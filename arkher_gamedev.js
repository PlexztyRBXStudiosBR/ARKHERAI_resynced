...
(function () {
  ...
  GD.loop = async function (cfg) {
    ...
    let stationRunId = null;
    const passo = (t, extra) => {
      const dado = Object.assign({ tipo: t }, extra || {});
      try { c.onPasso(dado); } catch (e) {}
      if (stationRunId && typeof ArkherStation !== 'undefined') {
        try {
          const estado = t === 'fim' ? (dado.ok ? 'completed' : 'failed')
            : (t === 'diagnostico' && ['nao-rodou', 'sem-saida', 'motor-desconhecido'].includes(dado.estado) ? 'failed'
              : (t === 'rodada' || t === 'codigo' || t === 'escrito' || t === 'rodou' || t === 'diagnostico' ? 'running' : undefined));
          ArkherStation.event(Object.assign({ type: 'gamedev_' + t, runId: stationRunId, state: estado }, dado));
          if (t === 'diagnostico') ArkherStation.checkpoint(stationRunId, {
            progress: c.maxRodadas ? Math.min(1, Number(dado.n || 0) / c.maxRodadas) : 0,
            metrics: { errors: (dado.erros || []).length, state: dado.estado || 'unknown' },
            message: dado.naoRodou || ''
          });
          if (t === 'fim') ArkherStation.updateRun(stationRunId, {
            status: dado.ok ? 'completed' : 'failed', artifacts: reg.arquivos.slice(), notes: dado.motivo || reg.motivo || ''
          });
        } catch (e) {}
      }
    };
    ...
    if (typeof ArkherStation !== 'undefined') {
      try {
        const sr = ArkherStation.createRun({ name: c.nome || ('game-dev ' + m.id), kind: 'game-dev-verification', base: m.id,
          notes: String(c.objetivo).slice(0, 240) });
        stationRunId = sr.id;
        ArkherStation.event({ type: 'gamedev_start', runId: stationRunId, task: m.id, state: 'running' });
      } catch (e) {}
    }
