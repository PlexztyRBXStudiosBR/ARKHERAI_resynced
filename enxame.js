  ...
  const ENX = {
    ...
    async agente(messages, o) {
      ...
      for (let volta = 0; volta <= maxVoltas; volta++) {
        ...
        let res;
        try {
          const contexto = o.pergunta || msgs.filter(m => m.role === 'user').map(m => m.content || '').join('\n');
          const gate = (typeof Guard !== 'undefined' && Guard.autorizarFerramenta)
            ? Guard.autorizarFerramenta(call, contexto) : { ok: true };
          if (!gate.ok) {
            res = { ok: false, err: 'Guard bloqueou a ferramenta: ' + gate.reason };
            try { if (typeof ArkherStation !== 'undefined') ArkherStation.event({ type: 'guard_tool_block', tool: call.tool, reason: gate.reason, source: 'enxame' }); } catch (x) {}
          } else res = await runTool(call, t => log('    ' + t));
        }
